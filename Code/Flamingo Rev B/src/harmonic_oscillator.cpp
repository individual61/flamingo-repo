...
{
  return -damping * v - springconstant * x + acc_ext;
...
{
  float k1, k2, k3, k4;
  k1 = eval_v_func(oldv, oldx, acc_ext, damping, springconstant);
  ...
  k4 = eval_v_func(oldv + k3, oldx, acc_ext, damping, springconstant);
...
// DAMPED HARMONIC OSCILLATOR. Returns object position as a fraction of max
// range, centered on 0, so maxrange is half the length of the strip
  ...
  if (first_iter == 1)  // first iteration
    {
      ...
      vel1 = get_next_v(vel0, pos0, acc_ext, DAMPING, SPRINGCONSTANT);
      ...
      if (timet - sendTime > 100)
        {
          ...
          Serial.print(((float)NUMPERSTRAND / 2.0) +
                       (pos1 / (MASS_REAL * 9.81 / SPRINGCONSTANT_REAL) - 1.0) *
                           ((float)NUMPERSTRAND / 2.0));
          Serial.print(F("\tBallPositionReal:\t"));
          Serial.print(pos1 / (MASS_REAL * 9.81 / SPRINGCONSTANT_REAL) - 1.0);
          Serial.print(F("\tVel:\t"));
          Serial.print(vel1);
          Serial.print(F("\tgetOffsetAccel:\t"));
          Serial.print(getOffsetAccel(GFACTOR));
          Serial.print(F("\tgetPhysicalOffsetAccel:\t"));
          Serial.println(getPhysicalOffsetAccel());

          sendTime = timet;
        }
    }
  return pos1 / (MASS_REAL * 9.81 / SPRINGCONSTANT_REAL) - 1.0;
}
